...
class DataLoader:
    ...
    def generate_synthetic_volatility(
        ...
        for t in range(1, n_samples):
            ...
            # GARCH dynamics: volatility = regime_mean + recent_shock + persistence
        ...
        # VIX usually correlates with volatility. Base ~ 15, spikes with vol.
        vix = 15 + (volatility * 100) * np.random.uniform(0.8, 1.2, size=n_samples)
        # Volume spikes when volatility spikes
    ...
    def load_yahoo_finance(ticker='AAPL', start_date='2020-01-01', end_date='2024-01-01', include_vix=True, include_volume=True):
        ...
        # Handle yfinance >= 0.2.40 which passes columns as MultiIndex
        if isinstance(data.columns, pd.MultiIndex):
            # Flatten or grab the first level
            cols = {
        ...
        if include_vix:
            print(f"Loading ^VIX data from Yahoo Finance...")
        ...
        print(f"[OK] Loaded {len(result)} trading days")
        return result
