...
class DataPreprocessor:
    ...
    def handle_missing_values(self, data: pd.DataFrame, column: str) -> pd.DataFrame:
        
        data = data.copy()
        missing_before = data[column].isna().sum()
        
        if missing_before == 0:
            print(f"[OK] No missing values in '{column}'")
            return data
    ...
    def handle_outliers(self, data: pd.DataFrame, column: str, method='iqr') -> pd.DataFrame:
    ...
    def compute_lagged_features(self, data: pd.DataFrame, column: str, n_lags: int = 5) -> pd.DataFrame:
        ...
        print(f"[OK] Created {n_lags} lagged features")
    ...
    def scale_features(self, X_train: np.ndarray, X_val: np.ndarray = None, X_test: np.ndarray = None) -> Tuple:
        ...
        print(f"[OK] Features scaled (fit on train only)")
        print(f"  - Train: mean={X_train_scaled.mean():.4f}, std={X_train_scaled.std():.4f}")
        
        return X_train_scaled, X_val_scaled, X_test_scaled
    
    def create_sequences(self, data: np.ndarray, seq_length: int = 30) -> Tuple:
...
class DataSplitter:
    ...
    @staticmethod
    def train_val_test_split(*arrays, train_ratio: float = 0.6, val_ratio: float = 0.2) -> Tuple:
        if not arrays:
        ...
        print(f"[OK] Data split (time-aware):")
