  ...
  def optimize_sarima(self, n_trials=20):
    ...
    def objective(trial):
      try:
        ...
        print(f"Trial {trial.number}: SARIMA({p},{d},{q})")
        model = SARIMAModel(
          order=(p, d, q),
  ...
  def optimize_exp_smoothing(self, n_trials=10):
    ...
    def objective(trial):
      try:
        ...
        print(f"Trial {trial.number}: trend={trend}, seasonal={seasonal}")
    ...
    study.optimize(objective, n_trials=n_trials, show_progress_bar=True)
    print("Best Exponential Smoothing:")
    print(f"Parameters: {study.best_params}")
    print(f"MAE: {study.best_value:.6f}")
    return study.best_params, study.best_value
  def optimize_prophet(self, n_trials=10):
    ...
    def objective(trial):
      try:
        ...
        print(f"Trial {trial.number}: changepoint={changepoint_scale:.4f}, "
  ...
  def optimize_lstm(self, n_trials=20):
    ...
    def objective(trial):
      try:
        ...
        print(f"Trial {trial.number}: units=({lstm_units_1},{lstm_units_2}), "
...
def main():
  ...
  print("Running Optuna optimizations")
